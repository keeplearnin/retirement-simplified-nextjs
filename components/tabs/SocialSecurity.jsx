...
export default function SocialSecurity() {
  ...
  return (
    <div className="fade-up">
      <InfoBox title="Social Security Is a Starting Point, Not a Plan" color="var(--warn)" bgColor="var(--warn-dim)">
      ...
      <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 32, marginTop: 16 }}>
        ...
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
            <Stat label="Monthly Benefit" value={`$${currentEstimate.monthly.toLocaleString()}`} sub={`Claiming at age ${startAge}`} color="var(--accent)" />
            <Stat label="Annual Benefit" value={`$${currentEstimate.annual.toLocaleString()}`} sub={`${fmt(currentEstimate.annual)} per year`} color="var(--accent)" />
          </div>
