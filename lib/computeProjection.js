...
export function computeProjection(plan) {
  ...
  const combined = incomeProjections.map((inc, i) => {
    ...
    const runWaterfall = (marginalRate) => {
      ...
      if (shortfall > 0 && inc.isRetired && liquidBalance + reDrawable > 0) {
    ...
    // Cash withdrawals are excluded from ordinary income: spending down a
    // savings-account balance is a return of principal, not taxable income
    // (the interest is already modeled via cashReturn). Only tax-deferred
    // accounts (401k, pension lump) create ordinary income when drawn.
    const provisionalTax = computeTax({
      filingStatus: yearFilingStatus,
      ordinaryIncome: baseOrdinaryIncome + w.w401k + w.wPension,
    ...
    let withdrawalRealEstate = w.wRealEstate;

    if (rmdAmount > 0 && withdrawal401k < rmdAmount) {
      withdrawal401k = rmdAmount;
    }

    // Cash principal withdrawals are tax-free (see provisional pass above).
    const totalOrdinaryIncome = baseOrdinaryIncome + withdrawal401k + withdrawalPension;
    ...
    // tax-deferred withdrawals. Salary, rental, and part-time are NOT
    // retirement income — they don't qualify for PA/IL/MS/etc exemptions.
    const retirementIncome =
      (inc.pension || 0) + (inc.annuity || 0) + withdrawal401k + withdrawalPension;
