...
export function computeProjection(plan) {
  ...
  const combined = incomeProjections.map((inc, i) => {
    ...
    //
    // KNOWN LIMITATION (Phase F target): for couples, bal401k is the household
    // sum and the divisor uses primary's age. If primary is 73 and spouse is
    // 65 with their own 401k, this over-RMDs because spouse shouldn't yet be
    // forced to draw. Per-spouse balance tracking + per-spouse RMD math is a
    // larger refactor — defer to survivor/age-gap polish in Phase F.
    let rmdAmount = 0;
