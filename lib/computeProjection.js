...
export function findBreakingShock(plan, { maxShock = 7, iterations = 7 } = {}) {
  ...
  const atMax = computeProjection({ ...plan, expectedReturn: Math.max(0, baseReturn - maxShock) });
  if (atMax.moneyLastsAge >= longevity) {
    return { shock: maxShock, lastsAge: atMax.moneyLastsAge, robust: true };
  }
  ...
  // Plan covers at zero shock, fails at maxShock — binary search the threshold.
  let lo = 0, hi = maxShock;
  let breaking = { shock: maxShock, lastsAge: atMax.moneyLastsAge };
  for (let i = 0; i < iterations; i++) {
    const mid = (lo + hi) / 2;
    const r = computeProjection({ ...plan, expectedReturn: Math.max(0, baseReturn - mid) });
